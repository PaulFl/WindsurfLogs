...
public func decodeWaypoints(waypoints: [Data]) -> [[CLLocationWrapper]] {
    var decodedWaypoints = [[CLLocationWrapper]]()
    for wp in waypoints {
        ...
        let wrappedLocation = CLLocationWrapper(location: location)
        
        
        if decodedWaypoints.count == 0 {
            decodedWaypoints.append([wrappedLocation])
        } else {
            if location.distance(from: decodedWaypoints.last!.last!.location) > CLLocationDistance(4000) || abs(location.timestamp.distance(to: decodedWaypoints.last!.last!.location.timestamp)) > TimeInterval(7200) {
                decodedWaypoints.append([wrappedLocation])
            } else {
                decodedWaypoints[decodedWaypoints.count-1].append(wrappedLocation)
            }
        }
    }
